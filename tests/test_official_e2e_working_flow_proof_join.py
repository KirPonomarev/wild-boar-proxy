...
def _router_output_working_flow_packet(source: dict[str, object]) -> dict[str, object]:
    packet = working_flow.build_codex_working_flow_delivery_proof_packet(
        source,
        [
    ...
    assert packet["status"] == "ok"
    assert packet["command_execution_delivery_surface_proven"] is True
    assert packet["command_execution_router_output_bound"] is True
    return packet
...
class OfficialE2EWorkingFlowProofJoinTests(unittest.TestCase):
    ...
    def test_positive_accepts_router_output_command_working_flow_delivery(self) -> None:
        real_hook = _integrated_packet()
        delivery = _router_output_working_flow_packet(real_hook)
        packet = _packet(real_hook=real_hook, delivery=delivery)
