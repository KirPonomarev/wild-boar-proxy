...
"""
    ...
    def test_settings_screen_is_readonly_with_safe_actions_and_deferred_controls(self) -> None:
        ...
        self.assertEqual(settings_markup.count('data-settings-card="true"'), 8)
        ...
        self.assertIn("web design UI", settings_markup)
        self.assertIn("AGPL-3.0-or-later", settings_markup)
        self.assertIn("Настройки доступны только для чтения", settings_markup + js)
        self.assertIn("Изменения требуют отдельной admitted command surface", settings_markup + js)
        ...
        self.assertIn("только метаданные", settings_markup)
