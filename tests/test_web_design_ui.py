...
"""
    ...
    def test_diagnostics_screen_is_support_artifact_only(self) -> None:
        ...
        self.assertIn("Сбои", html)
        self.assertIn("Успешные попытки", html)
        self.assertIn('class="chart-line failure"', html)
        self.assertIn('class="chart-line success"', html)
        self.assertIn("100%", html)
        self.assertIn("2 июн.", html)
...
"""
    ...
    def test_settings_screen_is_readonly_with_safe_actions_and_deferred_controls(self) -> None:
        ...
        self.assertIn('data-settings-subscreen-mode="section-only"', settings_markup)
        for section in [
            "runtime-mode",
            "client-launch",
            "data-storage",
        ...
        self.assertIn("Данные и хранение", settings_markup)
        self.assertIn("Probe & Recovery", settings_markup)
        self.assertIn("Account Defaults", settings_markup)
        ...
        self.assertIn('data-ui-action="set_mode_stable"', settings_markup)
        self.assertIn('data-ui-action="set_mode_managed"', settings_markup)
        self.assertIn('data-ui-action="stable_repair_plan"', settings_markup)
        self.assertIn('data-ui-action="launch_smoke"', settings_markup)
        self.assertIn('data-ui-action="launch_client_dispatch"', settings_markup)
        self.assertIn('data-ui-action="export_diagnostics"', settings_markup)
        self.assertIn("браузер не отправляет произвольные пути", settings_markup)
        self.assertIn("прямой доступ к файловой системе отложен", settings_markup)
        self.assertIn("нет bounded open-location контракта", settings_markup)
        self.assertIn("Support artifacts", settings_markup)
        self.assertIn("только метаданные командных пакетов", settings_markup)
        self.assertIn("Probe limits", settings_markup)
        self.assertIn("доступен только dry-run план ремонта", settings_markup)
        self.assertIn("Apply repair", settings_markup)
        self.assertIn("изменение конфигурации runtime вне границ контура", settings_markup)
        self.assertIn("изменение политики требует owner command surface", settings_markup)
        self.assertIn("изменение стартовой политики отложено", settings_markup)
        self.assertIn("сырой route config не отправляется из браузера", settings_markup)
        self.assertIn("Route operations", settings_markup)
        self.assertIn("Key setup", settings_markup)
        self.assertIn("секреты и токены не вводятся в UI", settings_markup)
        self.assertIn("нет полей ввода и просмотра", settings_markup)
        self.assertIn("не принимает исходный каталог из браузера", settings_markup)
