...
WEB_DESIGN_LIVE_SERVER = ROOT / "wild_boar_proxy" / "web_design_live_server.py"
...
class OwnerSurfaceEffectInventoryTests(unittest.TestCase):
    ...
    def test_native_window_ax_helpers_use_bounded_osascript_without_raw_subprocess(
        self,
    ) -> None:
        runner_calls = _call_names(_function(NATIVE_WINDOW_PROBE, "_run_osascript"))
        self.assertIn("run_bounded_process", runner_calls)
        self.assertEqual(set(), runner_calls & SUBPROCESS_PRIMITIVES)

        for name in (
            "_window_observation_via_ax",
            "_focus_custom_window_by_pid",
            "_ax_input_capable",
            "_ax_input_capable_by_name",
        ):
            with self.subTest(function=name):
    ...
    def test_web_design_safe_app_copy_helper_uses_bounded_runner_without_raw_subprocess(
        self,
    ) -> None:
        calls = _call_names(_function(WEB_DESIGN_LIVE_SERVER, "_run_safe_app_copy_bounded_helper"))
        self.assertIn("run_bounded_process", calls)
        self.assertEqual(set(), calls & SUBPROCESS_PRIMITIVES)
    ...
    def test_web_design_git_probe_file_status_uses_bounded_runner_without_raw_subprocess(
        self,
    ) -> None:
        calls = _call_names(_function(WEB_DESIGN_LIVE_SERVER, "_git_probe_file_status"))
        self.assertIn("run_bounded_process", calls)
        self.assertEqual(set(), calls & SUBPROCESS_PRIMITIVES)
