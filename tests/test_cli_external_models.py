...
class ExternalModelsCliTests(unittest.TestCase):
    ...
    def test_check_success_writes_verified_state_and_evidence(self) -> None:
        ...
        self.assertEqual(payload["data"]["declared_mode"], "api_only")
        self.assertEqual(payload["data"]["route_truth_status"], "pass")
        self.assertEqual(payload["data"]["executed_provider"], "openrouter")
        self.assertEqual(payload["data"]["executed_model"], "deepseek/deepseek-chat")
        self.assertFalse(payload["data"]["chatgpt_invoked"])
        self.assertTrue(payload["data"]["chatgpt_route_absent"])
        self.assertFalse(payload["data"]["fallback_used"])
        self.assertTrue(payload["data"]["selected_vs_executed_separated"])
        self.assertFalse(payload["data"]["browser_selector_used"])
        self.assertFalse(payload["data"]["ui_selector_claimed"])
    ...
    def test_live_format_check_calls_provider_once_without_state_or_evidence_writes(self) -> None:
        ...
        self.assertFalse(payload["data"]["chatgpt_invoked"])
        self.assertTrue(payload["data"]["chatgpt_route_absent"])
        self.assertFalse(payload["data"]["codex_or_chatgpt_route_invoked"])
        self.assertFalse(payload["data"]["fallback_used"])
        self.assertTrue(payload["data"]["selected_vs_executed_separated"])
        self.assertFalse(payload["data"]["browser_selector_used"])
        self.assertFalse(payload["data"]["ui_selector_claimed"])
        self.assertFalse(payload["data"]["proof_file_smoke_required"])
        self.assertEqual(payload["data"]["request_count"], 1)
