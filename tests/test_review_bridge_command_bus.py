...
def import_payload(**overrides: object) -> dict[str, object]:
    payload: dict[str, object] = {
        ...
        "review_surface": {"items": [{"id": "change-1", "kind": "exact_text"}]},
        "revision_session": {"mode": "review_only", "source": "packet"},
...
class ReviewBridgeCommandBusTests(unittest.TestCase):
    ...
    def test_import_review_packet_is_admitted_through_command_bus(self) -> None:
        ...
        self.assertEqual(surface["session_id"], "session-001")
        self.assertEqual(surface["baseline_hash"], "sha256:baseline")
        self.assertEqual(surface["source_packet_hash"], "sha256:packet")
    ...
    def test_clear_review_session_runs_through_command_bus(self) -> None:
        execute_review_command(self.store, "import_review_packet", payload=import_payload())
    ...
    def test_import_rejects_missing_fields_and_unknown_args(self) -> None:
        ...
        extra = execute_review_command(
            self.store,
            "import_review_packet",
            payload=import_payload(packet_text="{bad-idea}"),
        )

        self.assertEqual(missing["status"], "command_error")
        self.assertEqual(missing["machine_error_code"], "REVIEW_COMMAND_MISSING_ARGS")
