...
def build_chatgpt_plus_api_slot_truth_packet(
    ...
    *,
    ...
    return {
        ...
        "declared_mode": CUSTOM_CODEX_EXECUTION_MODE_CHATGPT_API,
        "route_truth_status": "pass" if slot_truth_proven else "blocked",
        ...
        "primary_executor": "chatgpt" if slot_truth_proven else "",
        "coding_executor": "deepseek" if slot_truth_proven else "",
        "primary_model_slot_lane": str(primary_slot.get("lane") or ""),
        "coding_agent_model_slot_lane": str(coding_slot.get("lane") or ""),
        "coding_agent_model_slot_provider": str(coding_slot.get("provider") or ""),
...
def build_api_only_executor_truth_packet(
    payload: Any,
    operator_status: dict[str, Any] | None,
    *,
    endpoint: str = DEFAULT_ENDPOINT,
    ...
    api_primary_slot_proven = (
        ...
        and primary_slot.get("status") == "bound"
        and primary_slot.get("slot_id") == "primary_model_slot"
        and primary_slot.get("lane") == API_ROUTE_MODEL_LANE
    ...
    coding_slot_not_bound_for_mode = (
        ...
        and coding_slot.get("slot_id") == "coding_agent_model_slot"
        and str(coding_slot.get("reason") or "") == "api_only_uses_primary_model_slot"
    )
    no_runtime_claims = all(
        server_truth_packet.get(field) is False
        for field in (
    ...
    executor_truth_proven = (
        server_truth_packet.get("status") == "ok"
        and server_truth_packet.get("model_selection_truth_proven") is True
        and execution_mode == CUSTOM_CODEX_EXECUTION_MODE_API_ONLY
        and api_primary_slot_proven
        and coding_slot_not_bound_for_mode
        and server_truth_packet.get("api_only_calls_chatgpt") is False
        and no_runtime_claims
        and no_browser_or_secret_exposure
    )
    machine_error_code = "OK" if executor_truth_proven else str(
        server_truth_packet.get("machine_error_code") or API_ONLY_EXECUTOR_TRUTH_BLOCKER
    )
    if not executor_truth_proven and machine_error_code == "OK":
        if execution_mode != CUSTOM_CODEX_EXECUTION_MODE_API_ONLY:
            machine_error_code = "API_ONLY_EXECUTOR_TRUTH_REQUIRES_API_ONLY_MODE"
        elif not api_primary_slot_proven:
            machine_error_code = "API_ONLY_EXECUTOR_PRIMARY_SLOT_NOT_API"
        elif not coding_slot_not_bound_for_mode:
            machine_error_code = "API_ONLY_EXECUTOR_CODING_SLOT_MUST_BE_UNBOUND"
        else:
            machine_error_code = API_ONLY_EXECUTOR_TRUTH_BLOCKER
    return {
        "schema_version": 1,
        "packet_kind": "api_only_executor_truth",
        "captured_at_utc": utc_now(),
        ...
        is True,
        "server_catalog_source": server_truth_packet.get("server_catalog_source") is True,
        "selected_api_model_server_issued": api_primary_slot_proven,
        "primary_model_slot": primary_slot,
        "coding_agent_model_slot": coding_slot,
        "api_primary_slot_proven": api_primary_slot_proven,
        "coding_agent_model_slot_not_bound_for_mode": coding_slot_not_bound_for_mode,
        "chatgpt_primary_slot_proven": False,
        "api_line_selected_as_executor": api_primary_slot_proven,
        "api_line_used_as_executor": api_primary_slot_proven,
