...
def build_repo_owned_default_launcher_script_payload() -> str:
    ...
    return "\n".join(
        [
            ...
            '  LAUNCH_ENV_LOCK_CLAIM="${LAUNCH_ENV_LOCK_DIR}.claim.$$"',
            '  LAUNCH_ENV_LOCK_TOKEN="$$:$(/bin/date +%s)"',
            "  acquire_launch_env_lock() {",
            '    case "$LAUNCH_ENV_LOCK_DIR" in /tmp/wbp-codex-launch-env-*.lock) ;; *) exit 9 ;; esac',
            '    [ "$(/usr/bin/dirname "$LAUNCH_ENV_LOCK_DIR")" = "/tmp" ] || exit 9',
            "    umask 077",
            '    printf "%s\\n" "$LAUNCH_ENV_LOCK_TOKEN" > "$LAUNCH_ENV_LOCK_CLAIM"',
            "    attempts=0",
            '    while ! /bin/ln "$LAUNCH_ENV_LOCK_CLAIM" "$LAUNCH_ENV_LOCK_DIR" 2>/dev/null; do',
            ...
            '      holder_token="$(/bin/cat "$LAUNCH_ENV_LOCK_DIR" 2>/dev/null || printf "")"',
            '      holder_pid="${holder_token%%:*}"',
            '      holder_epoch="${holder_token#*:}"',
            '      case "$holder_pid:$holder_epoch" in ""|*[!0-9:]*) exit 9 ;; esac',
            ...
            "        continue",
            "      fi",
            '      reclaim_dir="${LAUNCH_ENV_LOCK_DIR}.reclaim.${holder_pid}.${holder_epoch}"',
            '      if /bin/mkdir "$reclaim_dir" 2>/dev/null; then',
            '        current_token="$(/bin/cat "$LAUNCH_ENV_LOCK_DIR" 2>/dev/null || printf "")"',
            '        if [ "$current_token" = "$holder_token" ]; then',
            '          /bin/rm -f "$LAUNCH_ENV_LOCK_DIR"',
            "        fi",
            '        /bin/rmdir "$reclaim_dir" 2>/dev/null || true',
            "      else",
            "        attempts=$((attempts + 1))",
            '        [ "$attempts" -lt 160 ] || exit 9',
            "        sleep 0.25",
            "      fi",
            "    done",
            '    /bin/rm -f "$LAUNCH_ENV_LOCK_CLAIM"',
            "  }",
            "  release_launch_env_lock() {",
            '    /bin/rm -f "$LAUNCH_ENV_LOCK_CLAIM"',
            '    [ -f "$LAUNCH_ENV_LOCK_DIR" ] && [ ! -L "$LAUNCH_ENV_LOCK_DIR" ] || return 0',
            '    holder_token="$(/bin/cat "$LAUNCH_ENV_LOCK_DIR" 2>/dev/null || printf "")"',
            '    [ "$holder_token" = "$LAUNCH_ENV_LOCK_TOKEN" ] || return 0',
            '    /bin/rm -f "$LAUNCH_ENV_LOCK_DIR"',
            "  }",
            ...
            "    release_launch_env_lock",
            "  }",
            '  trap cleanup_launch_env EXIT HUP INT TERM',
            "  launch_codex_app() {",
            "    set_launch_env",
