...
| `jq '.lanes[] | select(.fixture.source_stage=="15" and .fixture.target_stage=="20") | (.commands[] | select(.command=="rollout stage advance 20 backend-reserve-advance-stage20-step --json") | .stdout_json.stage_advancement_result.delegated_evidence.postflight_summary.readiness_summary)' audit_results/step16_owner_surface_capture.json` | 0 | Fallback/rollback readiness summary shows `status=ready`, `reason=bounded_fallback_and_recovery_contracts_ready`. |
| `rg -n --no-heading '"command"\s*:\s*"python3 -m unittest -q tests\.test_cli -k installer"' audit_results/step18a_test_runs.json` | 0 | Installer command evidence is present in prior machine artifact. |
| `rg -n --no-heading '"command"\s*:\s*"python3 -m unittest -q tests\.test_cli -k legacy_import"' audit_results/step18a_test_runs.json` | 0 | Legacy import command evidence is present in prior machine artifact. |
| `rg -n --no-heading 'test_diagnostics_export_redacts_runtime_state_and_registry_secrets|test_rollout_evidence_capture_16_redacts_bundle_secrets' audit_results/step18a_test_runs.json` | 0 | Security command evidence is present in prior machine artifact. |
| `rg -n --no-heading '"command"\s*:\s*"python3 -m unittest -q tests\.test_cli -k package_experimental"' audit_results/step18a_test_runs.json` | 0 | License-signal command evidence is present in prior machine artifact. |
| `rg --files audit_results | rg -i 'telemetry|metric|metrics|two.?week|2w|14.?day|window'` | 0 | Only prior metrics artifacts are listed; complete 14-day machine window evidence is not detected. |
...
- Closed blockers in step22 scope: 0.
