# STEP-28 Closeout Report

- Generated at (UTC): `2026-05-07T08:21:05Z`
...
|---|---:|---|
| `python3 -m unittest -q tests.test_cli -k stage_advance_20` | 0 | `Ran 14 tests in 8.302s`; `OK`; emitted `sync-advance-stage20-inline-auth`, `sync-advance-stage20-contradicted`, `sync-advance-stage20-stable-auth-failure`. |
| `python3 -m unittest -q tests.test_cli.CliTests.test_rollout_stage_advance_20_fails_on_postflight_contradiction_after_promotion` | 0 | `Ran 1 test in 1.847s`; `OK`; emitted `sync-advance-stage20-contradicted`. |
| `python3 -m unittest -q tests.test_cli.CliTests.test_rollout_stage_advance_20_rolls_back_failed_stable_auth_materialization` | 0 | `Ran 1 test in 1.161s`; `OK`; emitted `sync-advance-stage20-stable-auth-failure`. |
...
| `rg --files audit_results | rg -i 'telemetry|metric|metrics|two.?week|2w|14.?day|window'` | 0 | listed metrics-window captures/reports including `step27b_metrics_window_capture.json`, `step26b_metrics_window_capture.json`, `step19b_metrics_capture.json`, and step20b-25b reports. |
